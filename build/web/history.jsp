...
<%
    /* Session check — servlet stores account under “acc” */
...
<head>
    ...
    <style>
        ...
        .history-container { max-width:1000px; margin:60px auto; background:#fff; border-radius:12px;
                             box-shadow:0 6px 16px rgba(0,0,0,0.1); padding:40px; }
    </style>
</head>
<body>
...
<div class="container mt-5 mb-5">
    ...
    <!-- DEBUG: contractMap = ${contractMap} -->

    <c:choose>
        <c:when test="${empty orders}">
            <div class="alert alert-info text-center">
                You have no approved rental orders at this time.
            </div>
        </c:when>
                ...
                <tbody>
                    <c:forEach var="order" items="${orders}" varStatus="loop">
                        <tr>
                                    ...
                                    <c:otherwise>
                                        ...
                                        <c:if test="${daysLeftMap[order.rentalOrderID] <= 14}">
                                            <small class="text-warning">
                                                ⚠ ${daysLeftMap[order.rentalOrderID]} day(s) left
                                            </small>
                                        </c:if>
                                    </c:otherwise>
                                </c:choose>
                            ...
                            <td>
                                <!-- Details -->
                                <a href="rental-detail?rentalOrderId=${order.rentalOrderID}"
                                   class="btn btn-outline-info btn-sm mb-1">Details</a>
                                ...
                                <c:if test="${!isFinalPaidMap[order.rentalOrderID]}">
                                    <form action="final-payment" method="post" class="d-inline">
                                        ...
                                        <button type="submit"
                                                class="btn btn-primary btn-sm mb-1">Pay&nbsp;Now</button>
                                    </form>
                                </c:if>
